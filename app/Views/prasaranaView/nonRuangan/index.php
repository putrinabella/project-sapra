            ...
            <div class="col-md-4 grid-margin stretch-card">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <a href="<?=site_url('prasaranaNonRuangan/'.$value->idIdentitasPrasarana) ?>"
...
$(document).ready(function () {
    $('#searchInput').on('input', function () {
        var namaPrasarana = $(this).val();

        $.ajax({
            type: 'POST',
            url: '<?= site_url('prasaranaNonRuangan/search') ?>',
            data: { namaPrasarana: namaPrasarana },
            dataType: 'json',
        ...
        if (results.length > 0) {
            $.each(results, function (index, value) {
                ...
                card += '<div class="card-body">' +
                    '<h5 class="card-title text-center">' +
                    value.namaPrasarana + ' (' + value.kodePrasarana + ')</h5>' +
                    '<p class="card-text mb-3">' +
                    '<span class="badge rounded-pill border border-primary text-primary">' +
                    ...
                    '</p>' +
                    '<a href="<?=site_url('prasaranaNonRuangan/') ?>' + value.idIdentitasPrasarana + '" class="btn btn-primary">Tampilkan Aset</a>' +
                    '</div></div></div>';
