            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="table-responsive">
                            <table class="table table-hover" id="dataTable">
                                ...
                                <tbody class="py-2">
                                    <?php foreach ($dataSemuaSarana as $key => $value) : ?>
                                    <tr style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle;">
                                        <td><input type="checkbox" class="form-check-input row-select"
                                                name="selectedRows[]" value="<?= $value->idRincianLabAset ?>"></td>
                        ...
                        <!-- <div class="row mb-3">
                            <label for="namaLab" class="col-sm-3 col-form-label">Laboratorium</label>
                            <div class="col-sm-9">
                        ...
                        <div class="row mb-3">
                            <label for="namaLab" class="col-sm-3 col-form-label">Gedung</label>
                            <div class="col-sm-9">
                                <div class="border rounded-2 p-2">
                        ...
                        <div class="row mb-3">
                            <label for="namaLab" class="col-sm-3 col-form-label">Lantai</label>
                            <div class="col-sm-9">
                                <div class="border rounded-2 p-2">
                        ...
                        <div class="row mb-3">
                            <label for="namaLab" class="col-sm-3 col-form-label">Luas</label>
                            <div class="col-sm-9">
                                <div class="border rounded-2 p-2">
                            ...
                            <tbody class="py-2">
                                ...
                                <tr style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle;">
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="<?= site_url('manajemenPeminjaman/new') ?>"
                                            class="btn btn-outline-primary me-2">Ajukan</a>
                                        <a href="#" class="btn btn-outline-primary upload-excel-btn"
                                            ...
                                            data-id-rincian-lab-aset="<?= $value->idRincianLabAset ?>">Ajukan
                                            Peminjaman</a>
                                        <?php else : ?>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered" id="newTable">
                                <thead>
                                    <tr class="text-center">
                                        <th style="width: 2%;"></th>
                                        <th style="width: 5%;">No.</th>
                                ...
                                <tbody class="py-2">
                                    <tr id="emptyMessageRow">
                                        <td colspan="6" class="text-center">Tambahkan barang untuk dipinjam</td>
                                    </tr>
                                </tbody>
                            </table>
                            <!-- <button id="showSelectedValues" class="btn btn-primary mt-3">Show Selected Values</button> -->
                            <button id="showSelectedRowsButton" class="btn btn-primary mt-3">Show Selected Rows</button>
    ...
    $(document).ready(function () {
        function moveSelectedRows() {
            ...
            selectedRowIds.forEach(function (rowId) {
                const selectedRow = $('#dataTable tbody').find('tr').filter(':has(:checkbox[value="' + rowId + '"])').clone();
                $('#newTable tbody').append(selectedRow);
            });

            $('#dataTable tbody').find('tr').filter(':has(:checkbox:checked)').remove();

            updateEmptyMessage();
        }

        $('#dataTable').on('change', 'input.row-select', function () {
            moveSelectedRows();
            if ($(this).is(':checked')) {
                const rowId = $(this).val();
                console.log('Checkbox checked, value:', rowId);
            }
        });

        $('#newTable').on('change', 'input.row-select', function () {
            const uncheckedRowIds = getUncheckedRowIds();

            uncheckedRowIds.forEach(function (rowId) {
                const uncheckedRow = $('#newTable tbody').find('tr').filter(':has(:checkbox[value="' + rowId + '"])').clone();
                $('#dataTable tbody').append(uncheckedRow);
            });

            $('#newTable tbody').find('tr').filter(':has(:checkbox:not(:checked))').remove();

            updateEmptyMessage();
        });

        function getSelectedRowIds() {
            const checkboxes = document.querySelectorAll('#dataTable input[name="selectedRows[]"]:checked');
        ...
        function getUncheckedRowIds() {
            const checkboxes = document.querySelectorAll('#newTable input[name="selectedRows[]"]:not(:checked)');
            const uncheckedRowIds = [];
            checkboxes.forEach(function (checkbox) {
                uncheckedRowIds.push(checkbox.value);
            });
            return uncheckedRowIds;
        }

        function updateEmptyMessage() {
            const newTableBody = $('#newTable tbody');
            const emptyMessageRow = $('#emptyMessageRow');

            if (newTableBody.find('tr').length > 1) {
                emptyMessageRow.hide();
            } else {
                emptyMessageRow.show();
            }
        }
        ...
        <div class="modal-content">
            ...
            <form action="<?=site_url('manajemenPeminjaman/addLoan') ?>" method="POST" enctype="multipart/form-data"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <input type="text" class="form-control" id="status" name="status" value="Peminjaman">
                    </div>
                    <div class="mb-3">
