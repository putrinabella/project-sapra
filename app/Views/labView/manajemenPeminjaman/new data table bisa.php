...
<script>
    $(document).ready(function () {
        ...
        $("#idIdentitasLab").on("change", function () {
            ...
            $.ajax({
                url: "<?= site_url('manajemenPeminjaman/getRincianLabAsetByLab') ?>",
                type: "POST",
                data: {
                ...
                success: function (response) {
                    $dataTableBody.empty();
                    if (response.length === 0) {
                        $dataTableBody.append("<tr><td colspan='10' class='text-center'>No data</td></tr>");
                    } else {
                        $.each(response, function (index, value) {
                            $dataTableBody.append("<tr>" +
                                "<td class='text-center'>" + (index + 1) + "</td>" +
                                "<td>" + value.kodeRincianLabAset + "</td>" +
                                "<td>" + value.kategoriAset + "</td>" +
                                "<td>" + value.namaAset + "</td>" +
                                "<td>" + value.status + "</td>" +
                                "<td>" + value.merek + "</td>" +
                                "<td>" + value.warna + "</td>" +
                                "</tr>");
                ...
                error: function () {
                    alert("Failed to retrieve data from tblRincianLabAset");
                }
            });
        });
