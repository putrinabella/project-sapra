...
class PrasaranaRuangan extends ResourceController
{
    ...
    public function new() {
        ...
        return view('prasaranaView/ruangan/new', $data);        
    }
    ...
    public function create() {
        ...
        if (!empty($data['idIdentitasSarana']) && !empty($data['tahunPengadaan']) && !empty($data['idSumberDana']) && !empty($data['kodePrasarana'])) {
            ...
            $totalSarana =  $this->prasaranaRuanganModel->calculateTotalSarana($data['saranaLayak'], $data['saranaRusak']);
            $data['totalSarana'] = $totalSarana;
            $this->prasaranaRuanganModel->insert($data);
            $this->prasaranaRuanganModel->setKodeAset();
            return redirect()->to(site_url('prasaranaRuangan'))->with('success', 'Data berhasil disimpan');
        } else {
            return redirect()->to(site_url('prasaranaRuangan'))->with('error', 'Semua field harus terisi');
        }
    }
    ...
    public function update($id = null) {
        if ($id != null) {
            ...
            if (!empty($data['idIdentitasSarana']) && !empty($data['tahunPengadaan']) && !empty($data['idSumberDana']) && !empty($data['kodePrasarana'])) {
                ...
                $this->prasaranaRuanganModel->update($id, $data);
                $this->prasaranaRuanganModel->updateKodeAset($id);
                return redirect()->to(site_url('prasaranaRuangan'))->with('success', 'Data berhasil diupdate');
            } else {
                return redirect()->to
                (site_url('prasaranaRuangan/edit/'.$id))->with('error', 'Id Sarana dan Lantai harus diisi.');
            }
        } else {
            return view('error/404');
        }
    ...
    public function edit($id = null) {
        if ($id != null) {
            ...
            if (is_object($dataPrasaranaRuangan)) {
                ...
                return view('prasaranaView/ruangan/edit', $data);
            } else {
                return view('error/404');
            }
        } else {
    ...
    public function delete($id = null) {
        $this->prasaranaRuanganModel->delete($id);
        return redirect()->to(site_url('prasaranaRuangan'));
    }

    public function trash() {
        $data['dataPrasaranaRuangan'] = $this->prasaranaRuanganModel->onlyDeleted()->getRecycle();
        return view('prasaranaView/ruangan/trash', $data);
    } 

    public function restore($id = null) {
        $this->db = \Config\Database::connect();
        if($id != null) {
            $this->db->table('tblPrasaranaRuangan')
                ->set('deleted_at', null, true)
                ->where(['idPrasaranaRuangan' => $id])
                ->update();
        } else {
            $this->db->table('tblPrasaranaRuangan')
                ->set('deleted_at', null, true)
        ...
        if($this->db->affectedRows() > 0) {
            return redirect()->to(site_url('prasaranaRuangan'))->with('success', 'Data berhasil direstore');
        } 
        return redirect()->to(site_url('prasaranaRuangan/trash'))->with('error', 'Tidak ada data untuk direstore');
    } 

    public function deletePermanent($id = null) {
        if($id != null) {
        $this->prasaranaRuanganModel->delete($id, true);
        return redirect()->to(site_url('prasaranaRuangan/trash'))->with('success', 'Data berhasil dihapus permanen');
        } else {
            $countInTrash = $this->prasaranaRuanganModel->onlyDeleted()->countAllResults();
        
            if ($countInTrash > 0) {
                $this->prasaranaRuanganModel->onlyDeleted()->purgeDeleted();
                return redirect()->to(site_url('prasaranaRuangan/trash'))->with('success', 'Semua data trash berhasil dihapus permanen');
            } else {
                return redirect()->to(site_url('prasaranaRuangan/trash'))->with('error', 'Tempat sampah sudah kosong!');
            }
        }
    } 
    ...
    public function createTemplate() {
        ...
        foreach ($data as $index => $value) {
            ...
            $exampleSheet->setCellValue('B'.($index + 2), $value->kodePrasaranaRuangan);
    ...
    public function import() {
        ...
        if($extension == 'xlsx' || $extension == 'xls') {
            ...
            foreach ($theFile as $key => $value) {
                ...
                $kodePrasaranaRuangan        = $value[1] ?? null;
                ...
                if ($kodePrasaranaRuangan !== null) {
                    $data = [
                        'kodePrasaranaRuangan' => $kodePrasaranaRuangan,
            ...
            return redirect()->to(site_url('prasaranaRuangan'))->with('success', 'Data berhasil diimport');
        } else {
            return redirect()->to(site_url('prasaranaRuangan'))->with('error', 'Masukkan file excel dengan extensi xlsx atau xls');
        }
    }
