...
class RincianLabAset extends ResourceController
{
    ...
    public function index() {
        $data['dataRincianLabAset'] = $this->rincianLabAsetModel->getData();
    ...
    public function restore($id = null) {
        $affectedRows = restoreData('tblRincianLabAset', 'idRincianLabAset', $id, $this->userActionLogsModel, 'Laboratorium - Rincian Lab Aset');
    ...
    public function deletePermanent($id = null) {
        $affectedRows = deleteData('tblRincianLabAset', 'idRincianLabAset', $id, $this->userActionLogsModel, 'Laboratorium - Rincian Lab Aset');
    
        if ($affectedRows > 0) {
            return redirect()->to(site_url('rincianLabAset'))->with('success', 'Data berhasil dihapus');
        } 
    
        return redirect()->to(site_url('rincianLabAset/trash'))->with('error', 'Tidak ada data untuk dihapus');
    }
    ...
    public function export() {
        ...
        header('Content-Disposition: attachment;filename=Laboratorium - Rincian Lab Aset.xlsx');
    ...
    public function createTemplate() {
        ...
        $spreadsheet->setActiveSheetIndex(0);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename=Rincian Lab Aset Example.xlsx');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
        exit();
    }
    ...
    public function generateSelectedLabQR($selectedRows) {
        ...
        $dompdf->render();
        $filename = 'Sarana - Selected QR Code Rincian Lab Aset .pdf';
    ...
    public function generateLabQRDoc() {
        ...
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        $filename = 'Sarana - QR Code Rincian Lab Aset.pdf';
        $dompdf->stream($filename, array("Attachment" => false));
    }
    ...
    public function generatePDF() {
        ...
        $dataAsetRusak = $this->rincianLabAsetModel->getDataRusak();
        $dataAsetHilang = $this->rincianLabAsetModel->getDataHilang();
        $title = "REPORT RINCIAN ASET LABORATORIUM";
        ...
        $pdfData = pdfRincianLabAset($dataAsetBagus, $dataAsetRusak, $dataAsetHilang, $title);
    
        
        $filename = 'Laboratorium - Rincian Lab Aset' . ".pdf";
